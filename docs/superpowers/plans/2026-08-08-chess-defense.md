...
- **계획 결함 2 — `buff.ts` 문서 주석 오류.** "퀸 자신의 칸도 커버리지에 포함되지만 자기 자신에
  ...
  docstring만 수정했다.
- **스펙 결함(계획 결함과 구분 — 우리가 고칠 대상이 아니다) — 스펙 9.4 "폰 3개로 웨이브5 보스
  처치"는 시뮬레이션으로 반증됨.** 기하학적 이유: 폰 `(f, r)`은 `(f-1, r+1)`과 `(f+1, r+1)` 두
  칸만 공격하므로 보스가 서 있는 한 칸을 동시에 때릴 수 있는 폰은 최대 2마리뿐이고, 보스가
  1랭크에 도달하면 0랭크가 없어 48초 완주 구간의 마지막 6초는 어떤 폰도 커버하지 못한다.
  실측(`tests/simulation.test.ts`): 완벽 추격 폰 2마리는 336 데미지, 3번째 폰의 기여분은
  정확히 0(단언으로 고정, 관찰이 아님), 룩(보스 파일) +80, 비숍(대각선) +12 — 혼합 빌드 총합
  428 vs 필요 420(마진 8), 비용 1,000G 대 골드 상한 2,108G. 구현을 스펙에 맞춰 바꾸지 않았다 —
  엔진은 스펙이 명시한 기물 기하를 그대로 구현했고, 이 격차는 스펙 자체의 9.5 플레이테스트
  항목으로 이관되는 밸런스 표 문제다. 부차 발견 한 줄: 보드는 8파일인데 배치 가능 랭크는
  7개뿐이라 룩 8개가 각자 전용 랭크를 가질 수 없고, 룩은 자기 파일·랭크를 완전 관통하므로
  강제된 랭크 공유가 파일 간 시너지를 만들어 후반 웨이브 방어가 파일별 단순 계산보다 실제로
  더 저렴해진다.
