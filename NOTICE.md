...
## 공격 사운드 — `src/assets/sounds/`

체스 디펜스 v1.2에서 추가된 **공격 사운드 전용** 첫 슬라이스(웨이브/보스/승리 등 다른 사운드는
아직 없음 — `chess-defense-game-spec.md` §10.1/§7.6 참조)에 쓰인 효과음이다. [Kenney](https://kenney.nl/)가
...
저장소에서는 게임 내 역할이 드러나는 이름으로 바꿔 두었다. 4개 파일 모두 Ogg Vorbis 코멘트
...
다만 이 헤더에는 **원본 파일명 필드가 없어**, 4개 파일 각각이 Kenney의 어느 팩·어느 원본
파일명에 대응하는지는 이 저장소 안의 정보만으로 다시 추적할 수 없다 — 정확하지 않은 원본
파일명을 지어내 적어두는 대신 이 사실을 그대로 기록해 둔다.

| 저장소 경로 | 게임 내 역할 | 원본 Kenney 파일명 |
|---|---|---|
| `src/assets/sounds/fire-pawn.ogg` | 폰 `attack` 큐 | 불명 (Vorbis 코멘트에 파일명 필드 없음) |
