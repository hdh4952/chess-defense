...
const C = {
  ...
  // 퀸 8방향 라인. 예전 0.55는 스폰 표식(rgba(200,60,50,0.10))과 같은 함정에 살짝 걸렸다 —
  // 금색이 나무색 보드와 색상(hue)이 가까워, 특히 어두운 칸 위에서 대비가 옅어졌다(재검토
  // Item 5, ΔY 10.0→4.1 측정). 완전히 안 보이는 수준은 아니었지만(실측 스크린샷으로 확인),
  // 값싸게 개선할 여지가 있어 알파만 0.70으로 올렸다 — 색상은 그대로라 튀지 않으면서
  // 대비 폭은 되살아난다.
