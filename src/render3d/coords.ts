...
/**
 * ★★ **뷰는 두 가지다** (v1.36 — 모바일, 사용자 요청).
 *
 * 위 `VIEW_W`의 논리는 "가로만 넓히면 보드가 한 픽셀도 줄지 않는다"였다. **그 전제는 화면이
 * 보드보다 가로로 넉넉할 때만 참이다.** 폰에서는 화면 폭이 먼저 동나므로, 킹 자리 180px은
 * 보드에서 그대로 떼어 낸 몫이 된다 — 실측 390px 폭에서 판이 256px, 화면의 66%였다.
 *
 * 그래서 좁은 화면에서는 **판만 담는 정사각 뷰**를 쓴다. 킹은 3D에서 빠지고, 그 자리가 하던
 * 일(플레이어 체력)은 판 바로 아래 DOM 막대가 맡는다(ui/layout.ts의 `#board-hp`).
 *
 * ⚠️ **뷰는 판이 시작될 때 한 번 정해지고 그 뒤로 바뀌지 않는다.** 카메라 프레이밍이
 * 생성 시점에 한 번 수렴하고(scene.ts의 fitCamera) 킹 메시도 그때 한 번 세워지기 때문이다.
 * 폰은 회전해도 둘 다 좁은 화면이라 실제로 문제가 되지 않고, 데스크톱에서 창을 좁혀도
 * 판은 fitBoardSize가 알아서 줄여 준다 — 킹 자리가 남아 있을 뿐이다.
 */
export interface BoardView {
  /** 캔버스(=뷰)의 가로·세로. 드롭 판정의 정규화와 오버레이 좌표계가 이 값을 쓴다. */
  readonly w: number;
  readonly h: number;
  /** 플레이어 킹을 판 밖에 세우는가. false면 체력은 DOM 막대가 말한다. */
  readonly king: boolean;
  /**
   * 프레이밍 여유. 좁은 뷰에서 더 꽉 채우는 이유는, 그 화면에서는 **여백 1%가 곧 판 크기**라
   * 서다 — 넓은 뷰의 0.94를 그대로 쓰면 정사각 뷰로 바꾼 이득의 상당수가 여백으로 돌아간다.
   */
  readonly margin: number;
}

/** 데스크톱 — 판 오른쪽에 킹이 선다 (v1.28). */
export const WIDE_VIEW: BoardView = { w: VIEW_W, h: VIEW_H, king: true, margin: 0.94 };

/** 좁은 화면 — 판만 담는다 (v1.36). 체력은 판 아래 DOM 막대가 말한다. */
export const COMPACT_VIEW: BoardView = { w: BOARD_W, h: BOARD_H, king: false, margin: 0.98 };
