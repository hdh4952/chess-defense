...
export class DragController {
  ...
  private moveGhost(e: PointerEvent): void {
    ...
    //
    // ★ **−140% → −100%** (v1.36, 사용자 피드백: "기물 아이콘이 내가 원하는 곳보다 상단에
    //   보여서 배치하기 어려워"). 규칙이 이제 한 문장으로 말해진다: **기물의 발이 손끝에
    //   선다.** −140%는 고스트 **아래에도** 높이의 40%만큼 빈 공간을 두는 값이라, 판정점이
    //   그림에서 그만큼 떨어져 "보이는 곳"과 "놓이는 곳"이 눈으로 이어지지 않았다.
