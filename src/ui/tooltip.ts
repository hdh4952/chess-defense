...
export function updateTooltip(
  ...
  if (t.slow) {
    ...
    // ★ 이 기물 자신의 티어로 유도한다 — T3 나이트에 "−30%"라고 적으면 그 자리에서 거짓말이다.
    rows.push(`L자 8칸의 적 이동속도 −${slowPercent(p.tier)}% (지속 · 8랭크 포함)`);
    // ★ 규칙의 두 축을 함께 말한다. v1.13부터 **강화는 듣고 중첩은 안 듣는다** — 이 구분을
    // 알려주지 않으면 플레이어는 나이트를 겹쳐 놓아 더 느리게 만들려고 한다(효과 없음).
    // 다음 단계 수치를 함께 보여주는 것이 합성 판단에 필요한 유일한 정보다.
    rows.push(
      p.tier < CONFIG.merge.maxTier[p.type]
        ? `합성하면 −${slowPercent(p.tier + 1)}% · 겹쳐 놓아도 중첩되지 않는다`
        : '최대 단계 · 겹쳐 놓아도 중첩되지 않는다',
    );
  }
