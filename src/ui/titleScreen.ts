...
/**
 * ⚠️⚠️ **v1.37에서 이 화면은 보관됐다 — 지금은 화면에 뜨지 않는다** (사용자 결정: "현재 로비
 * 페이지가 너무 글이 많고 가독성이 안좋아서 일단 그대로 보관해두고 새로운 로비 페이지
 * 제작하자"). 지금 뜨는 것은 `ui/lobby.ts`다.
 *
 * **지우지 않고 그대로 둔 것이 요점이다**(사용자 요청). 되살리려면 `main.ts`의 호출 한 줄을
 * `createTitleScreen(app, …)`으로 되돌리면 되고, `#title-*` CSS와 이 파일의 테스트도 함께
 * 남아 있다. 그래서 아래 주석의 "시작 화면"은 **그때의 화면**을 말한다.
 *
 * ⚠️ 지금 닿을 수 없게 된 기능이 하나 있다: **스킨 선택**(v1.19). 고른 스킨은 localStorage에
 * 남아 게임 화면에 계속 적용되지만, 바꾸는 자리는 이 화면뿐이었다.
 *
 * ---
 *
