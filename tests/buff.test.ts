...
import { drawPiece, emptySquares, grantPiece } from '../src/core/economy';
...
import { boardPiece, gachaRng, cleanState } from './helpers';
 ...
 * **"떨어진 그 순간의 값"이 곧 그 기물의 첫 화력**이다 — drawPiece/grantPiece가 recalcQueenBuffs를
...
describe('스폰 즉시 버프 반영 (v1.12)', () => {
  it('퀸 라인 위에 스폰된 구매 기물은 즉시 버프를 받는다', () => {
    ...
    const rook = drawPiece(s, events, gachaRng('rook', 0.42))!;   // 빈 칸이 하나뿐이라 위치 난수와 무관
  ...
  it('보드가 꽉 차면 스폰이 없고 기존 버프도 그대로다', () => {
    ...
    s.gold = CONFIG.pieces.rook.cost;
    const events: GameEvent[] = [];
    expect(drawPiece(s, events, gachaRng('rook'))).toBeNull();
